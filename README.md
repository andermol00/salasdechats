...
**GIFs animados sin API key**
- 15 GIFs animados incluidos en `public/gifs/` (generados por `scripts/make-gifs.sh`).
- No dependen de GIPHY ni de ningún servicio externo: funcionan offline y pesan ~104 KB en total.
...
El proyecto **no usa ninguna API key**:

- Los GIFs viven en `public/gifs/` y se generan con `bash scripts/make-gifs.sh`
  (dibujados con primitivas de ImageMagick y animados en 8 cuadros con fondo
  transparente).
- El listado permitido está en `src/lib/gifs.ts`.
- Las peticiones se envían con el formato `gif:<id>` y el servidor valida contra
  la lista, así que nadie puede enviar un GIF inyectado.
- Para añadir uno: dibuja un nuevo `.gif` en `public/gifs/` y añade su entrada a
  `GIFS` en `src/lib/gifs.ts`.
